...
async def check_notifications(db: AsyncSession, category_id: int, user_id: models.User, group_id: Union[int, None]):
    user = await crud.get_user(db, user_id=user_id)
    limits = await crud.check_limit(db, category_id, user_id=user.id, group_id=group_id)
    ...
    for limit in limits:
        if limit.percentage > 0.8 and limit.percentage < 0.95 and limit.n20_sent_at.month != datetime.date.month:
            send_notification(registration_token=user.notifications_token, 
                              title=f"{limit.category} limit status", 
                              body=f"{user.username} your limit for {limit.category} is at {round(limit.percentage*100, 2)}% ({limit.payments_sum}).")
        elif limit.percentage > 0.95 and limit.percentage < 1.0 and limit.n05_sent_at.month != datetime.date.month:
            send_notification(registration_token=user.notifications_token, 
                              title=f"{limit.category} almost reached the limit", 
                              body=f"{user.username} you've almost exceeded your limit for {limit.category}. Limit is at {round(limit.percentage*100, 2)}% ({limit.payments_sum}).")
